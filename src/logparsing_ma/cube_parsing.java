...
//    static String FILE = "C:"+SEP+"LOGS"+SEP+"FILES"+SEP+"Cube_ElasticSearchLogs"+SEP+"cube_log
    ...
    static String[] REGEXS = new String[]{  "\"CUBE\",\"(\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2},\\d{3})\".+\"(.+)\".+\"(\\d+)\".+\"L-JBOSS-.+SELECT \\{(.+)\\} ON COLUMNS.+WHERE \\( (.+) \\) \",\".+\""};
...
//                                         ,  "\"CUBE\",\"(\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2},\\d{3})\".+\"(.+)\".+\"L-JBOSS-.+SELECT \\{(.+)\\} ON COLUMNS.+WHERE \\( (.+) \\) \",\".+\"
    ...
    static void openCube() throws IOException {
        ...
        try (BufferedReader cube = new BufferedReader(new FileReader(FILE))) {
            ...
            int groupCount, i = 0, check = 0, x = 0;
                    ...
                    if (matcher.matches()) {
                        if (matcher.groupCount() == 5){
                            ...
                            if (matcher.group(2).matches(".*\\[time\\]\\.\\[Day Name\\]\\.&\\[\\d+\\] : \\[time\\]\\.\\[Day Name\\]\\.&\\[\\d+\\]:*")) {
                                ...
                                range_filter = matcher.group(5).replaceAll(".*\\{\\[time\\]\\.\\[Day Name\\]\\.&\\[(\\d+)\\]\\}", "$1 ;");
                            } else if (matcher.group(5).matches(".*\\{\\[time\\]\\.\\[Month Name\\]\\.&\\[\\d+\\]\\}.*")) {
                                range_filter = matcher.group(5).replaceAll(".*\\{\\[time\\]\\.\\[Month Name\\]\\.&\\[(\\d+)\\]\\}", "$1 ;");
                            } else if (matcher.group(5).matches(".*\\{\\[time\\]\\.\\[Day Name\\]\\.&\\[\\d+\\] : \\[time\\]\\.\\[Day Name\\]\\.&\\[\\d+\\]\\}.*")) {
                                range_filter = matcher.group(5).replaceAll(".*\\{\\[time\\]\\.\\[Day Name\\]\\.&\\[(\\d+)\\] : \\[time\\]\\.\\[Day Name\\]\\.&\\[(\\d+)\\]\\}.*", "$1 ; $2 ");//+";"+matcher.group(3);
                            } else if (matcher.group(5).matches(".*\\{\\[time\\]\\.\\[Month Name\\]\\.&\\[\\d+\\] : \\[time\\]\\.\\[Month Name\\]\\.&\\[\\d+\\]\\}.*")) {
                                range_filter = matcher.group(5).replaceAll(".*\\{\\[time\\]\\.\\[Month Name\\]\\.&\\[(\\d+)\\] : \\[time\\]\\.\\[Month Name\\]\\.&\\[(\\d+)\\]\\}.*", "$1 ; $2");//+";"+matcher.group(3);
                            } else {
                                range_filter = ";";
                            }
                            range_filter = range_filter.replace(",  {}", "");

                            range_filter_t = "";
                            term_filter = "publisher_id "+matcher.group(3)
                                    .replace("}", "")
                                    .replace("{", "")
                                    .replace("[", "")
                                    .replace("]", "")
                                    .replace("&", "");//"PublisherID : "+matcher.group(3);//+matcher.group(5);
